...
export const KPICard: React.FC<KPICardProps> = ({ label, value, comparison, icon, accentColor }) => {
  ...
  return (
    <div className={`bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:translate-y-[-2px] transition-all duration-300 relative overflow-hidden flex flex-col justify-between min-h-[135px] ${borderStyles[accentColor]}`}>
      <div>
        <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-[0.12em] mb-1.5">{label}</p>
        <h3 className={`text-3xl font-black tracking-tight ${textStyles[accentColor]} leading-none`}>
          {value.toString().padStart(2, "0")}
        </h3>
      </div>
      <div className="flex items-center justify-between mt-4 pt-2 border-t border-slate-50">
        <span className="text-[11px] font-semibold text-slate-400 font-sans tracking-tight">{comparison}</span>
        <div className={`p-2 rounded-xl transition-transform duration-300 ${iconBgStyles[accentColor]}`}>
          {icon}
        </div>
      </div>
    </div>
